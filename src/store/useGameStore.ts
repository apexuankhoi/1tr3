...
// Detect Expo Go (notifications limited in Expo Go)
...
try {
  ...
  console.log('[Push] Notification handler setup failed (expected in some environments)');
}

export interface PotData {
  id: string; // e.g., 'floor1_pot1'
  floorId: number;
  hasPlant: boolean;
  waterLevel: number;
...
const GROWTH_DURATION_MS = 3600000; // 1 hour in milliseconds

// Initial default pots
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  waterPot: (potId) => {
    set((state) => ({
      pots: state.pots.map(pot => {
        ...
        const newWater = Math.min(1, pot.waterLevel + 0.5);
  ...
  fertilizePot: (potId) => {
    set((state) => ({
      pots: state.pots.map(pot => {
        if (pot.id !== potId || !pot.hasPlant || pot.growingUntil > 0) return pot;
        const newFertilizer = Math.min(1, pot.fertilizerLevel + 0.5);
  ...
  syncGarden: async () => {
    ...
    if (userId) {
      try {
        ...
        console.log("[Garden] Đã đồng bộ khu vườn lên server");
      } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (savedPots && savedPots.length > 0) {
          ...
          set({ pots });
          console.log(`[Garden] Đã load ${pots.length} chậu cây từ server`);
        } else {
          const defaultPots = generateInitialPots();
          set({ pots: defaultPots });
          await gardenService.savePots(userId, defaultPots, data.seeds ?? 2);
        }
      } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (savedPots && savedPots.length > 0) {
          ...
          const defaultPots = generateInitialPots();
          set({ pots: defaultPots });
          await gardenService.savePots(data.id, defaultPots, data.seeds ?? 2);
        }
      } catch (error) {
  ...
  registerPushToken: async () => {
    if (isExpoGo) {
      console.log('[Push] Skipping push registration on Expo Go');
      return;
    }
    try {
      ...
      console.log('[Push] Push registration skipped:', (error as any)?.message);
    }
  },
  ...
  scheduleWaterReminder: async () => {
    try {
      ...
      console.log('[Push] Scheduled local notification');
    } catch (error) {
